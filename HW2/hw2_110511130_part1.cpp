...
const int M = 3;
const int EPOCH = 5;
const double WEIGHT_INIT = 5;
const int TRAIN_TOTAL = 1300;
const int TEST_TOTAL = 750;
vector<int> TRAIN_NUM = {300, 250, 350, 400};
vector<int> TEST_NUM = {200, 300, 150, 100};
			...
			while(ss>>str_temp){
				if(i==0)
					...
					if(filename == "HW2_training.csv")
						mean[i-1][int(t[j])] += stof(str_temp);
				}
				i++;
				if(i==FEATURE+1){
					j++;
...
void outputNum(double n, string filename){
	ofstream output;
	output.open("./output_part1/"+filename);
	output<<n<<endl;
	output.close();
}
	...
	for(int i=0;i<FEATURE;i++){
		for(int j=0;j<FEATURE;j++){
			for(int k=0;k<TRAIN_TOTAL;k++){
				covariance[i][j] += (training_x[i][k]-mean[i][4]) * (training_x[j][k]-mean[j][4]);
			}
...
void calculateConfusion(vector<vector<double>>& confusion, vector<double> prediction, vector<double> target_data, int input_num){
	for(int i=0;i<input_num;i++){
		confusion[target_data[i]][prediction[i]]++;
	}
}
...
void DiscriminativeModel::transformTarget(vector<double> target){
	for(int i=0;i<TRAIN_TOTAL;i++){
		target_transformed[target[i]][i] = 1;
	}
...
void DiscriminativeModel:: predict(vector<vector<double>> input_data, vector<double> target_data, string label){
	...
	for(int i=0;i<input_num;i++){
		...
		prediction[i] = 0;
		for(int j=1;j<CLASS;j++){
			if(probability_predict[j][i]>probability_predict[prediction[i]][i]){
...
int main(){
	...
	/*----------output data----------*/
	// output2DVector(training_x, FEATURE, TRAIN_TOTAL, "training/training_x.txt");
	// output2DVector(testing_x, FEATURE, TEST_TOTAL, "testing/testing_x.txt");
	// output1DVector(training_t, TRAIN_TOTAL, "training/training_t.txt");
	// output1DVector(testing_t, TEST_TOTAL, "testing/testing_t.txt");
