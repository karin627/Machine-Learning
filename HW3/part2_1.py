...
def convert_target(targets):
  new_targets = np.zeros((targets.shape[0], 4))
  for i in range(targets.shape[0]):
    new_targets[i][int(targets[i])] = 1
  return new_targets
...
train_targets = torch.tensor(train_file['Team'].values).to(torch.long)
test_file = pd.read_csv('HW2_testing.csv')
test_features = torch.tensor(test_file[['Offensive', 'Defensive']].values).to(torch.float32)
test_targets = torch.tensor(test_file['Team'].values).to(torch.long)
